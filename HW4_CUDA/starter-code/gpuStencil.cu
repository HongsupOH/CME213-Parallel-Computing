...
void gpuStencilGlobal(float* next, const float* __restrict__ curr, int gx, int nx, int ny,
    ...
    int ind = gid_x + gx*gid_y;

    if(tid_x < nx && tid_y < ny){
       next[ind] = Stencil<order>(curr+ind,gx,xcfl,ycfl);
    }
}
...
double gpuComputationGlobal(Grid& curr_grid, const simParams& params) {
    ...
    // choose block size as 192 threads
    ...
    for(int i = 0; i < params.iters(); ++i) {
        ...
        switch (order){
            case 2:
                gpuStencilGlobal<2><<<grids,blocks>>>(next_grid.dGrid_, curr_grid.dGrid_,gx,nx,ny,xcfl,ycfl);
                break;
            case 4:
                gpuStencilGlobal<4><<<grids,blocks>>>(next_grid.dGrid_, curr_grid.dGrid_,gx,nx,ny,xcfl,ycfl);
                break;
            case 8:
                gpuStencilGlobal<8><<<grids,blocks>>>(next_grid.dGrid_, curr_grid.dGrid_,gx,nx,ny,xcfl,ycfl);
                break;
            default:
...
double gpuComputationBlock(Grid& curr_grid, const simParams& params) {
    ...
    const int nx = params.nx();
    const int ny = params.ny();
    const double xcfl = params.xcfl();
    const double ycfl = params.ycfl();
    const int gx = params.gx();
    const int order = params.order();

    // choose block size as 256 threads
    const int thread_num = 256;
    const int block_x = 32;
    const int block_y = thread_num/block_x;
    dim3 blocks(block_x,block_y); //2D block (32,8)

    // compute stride in y directin of grid
    ...
    for(int i = 0; i < params.iters(); ++i) {
        ...
        switch (order){
            case 2:
                gpuStencilBlock<2,numYPerStep><<<grids,blocks>>>(next_grid.dGrid_, curr_grid.dGrid_,gx,nx,ny,xcfl,ycfl);
                break;
            case 4:
                gpuStencilBlock<4,numYPerStep><<<grids,blocks>>>(next_grid.dGrid_, curr_grid.dGrid_,gx,nx,ny,xcfl,ycfl);
                break;
            case 8:
                gpuStencilBlock<8,numYPerStep><<<grids,blocks>>>(next_grid.dGrid_, curr_grid.dGrid_,gx,nx,ny,xcfl,ycfl);
                break;
            default:
                fprintf(stderr, "%s\n", "Unknown order specified.");
        }
