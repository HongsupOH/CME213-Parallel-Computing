                                         ...
                                         uint numBits, uint startBit, uint blockSize)
{
    ...
    for(int i = 0; i<numBlocks;++i){
        for (int j = 0;j<blockSize;++j){
            uint k = keys[i*blockSize+j]>>startBit;
            uint bucket = k&((1<<numBits)-1);
            //uint bucket = mask&(keys[i*blockSize +j]);
                                  ...
                                  uint numBuckets)
{
    ...
    std::vector<uint> copy(numBuckets,0);
#pragma omp parallel for
    for(int i=0;i<numBuckets;i++){
        copy[i] = globalHisto[i];
    }
    for(int i=1;i<numBuckets;i++){
        copy[i] += copy[i-1];
    }
    for(int i=0;i<numBuckets-1;i++){
        globalHistoExScan[i+1] = copy[i];
    }
...
#pragma omp single
    for(int block = 0;block<numBlocks;++block){
        for(int bucket = 0;bucket<numBuckets;++bucket){
            int ind = numBuckets*block + bucket;
            if(block==0){
                blockExScan[ind] = globalHistoExScan[ind];
            }
            else{
                int pre_ind = numBuckets*(block-1) + bucket;
                blockExScan[ind] =  blockExScan[pre_ind] + blockHistograms[pre_ind]; 
            }
        }
    }
...
#pragma omp single
    for(int i = 0; i<numBlocks;++i){
        ...
        for (int j = 0;j<blockSize;++j){
            uint val = keys[i*blockSize+j];
            uint bucket = (keys[i*blockSize+j]>>startBit)&((1<<numBits)-1);
            uint ind = blockExScan[i*numBuckets+bucket];
            sorted[ind+offset[bucket]]=val;
            offset[bucket]++;
        }
    }
}
...
int main()
{
    ...
    std::vector<uint> jNumBlock = {1,2,4,8};
